...
from tool_sandbox.roles.comap_agent import CoMAPAgent
...
def run_scenario(
    ...
    *,
    ...
    previous_comap_scenario_name = os.environ.get("COMAP_SCENARIO_NAME")
    if agent_type == RoleImplType.CoMAP:
        os.environ["COMAP_SCENARIO_NAME"] = name
    roles = {
    ...
    try:
        ...
        for role in roles.values():
            role.teardown()
        if agent_type == RoleImplType.CoMAP:
            if previous_comap_scenario_name is None:
                os.environ.pop("COMAP_SCENARIO_NAME", None)
            else:
                os.environ["COMAP_SCENARIO_NAME"] = previous_comap_scenario_name
